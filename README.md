# Minecraft Legacy Console Edition Soundpack

**Authentic Legacy Console Edition music**  ripped directly from the leaked console source and ported for PC use.

This pack contains the original music tracks as they existed in **Minecraft Legacy Console Edition** (Xbox 360, PS3, Wii U, Switch, etc.) before many were changed, removed, or renamed in later versions.

### What's Included

##Game Music

**calm1 → calm3**
- calm1.ogg → Minecraft
- calm2.ogg → Clark
- calm3.ogg → Sweden

**creative1 → creative6**
- creative1.ogg → Biome Fest
- creative2.ogg → Blind Spots
- creative3.ogg → Haunt Muskie
- creative4.ogg → Aria Math
- creative5.ogg → Dreiton
- creative6.ogg → Taswell

**hal1 → hal4**
- hal1.ogg → Subwoofer Lullaby
- hal2.ogg → Living Mice
- hal3.ogg → Haggstrom
- hal4.ogg → Danny

**menu1 → menu4**
- menu1.ogg → Mutation
- menu2.ogg → Moog City 2
- menu3.ogg → Beginning 2
- menu4.ogg → Floating Trees

**nether1 → nether4**
- nether1.ogg → Concrete Halls
- nether2.ogg → Dead Voxel
- nether3.ogg → Warmth
- nether4.ogg → Ballad of the Cats

**nuance1 + nuance2**
- nuance1.ogg → Key
- nuance2.ogg → Oxygène

**piano1 → piano3**
- piano1.ogg → Dry Hands
- piano2.ogg → Wet Hands
- piano3.ogg → Mice on Venus

**The End**
- the_end_dragon_alive.ogg → First half of "The End" (Dragon fight)
- the_end_end.ogg → Second half of "The End" (credits)

##Music Discs

All .ogg files taken directly from the Legacy Console leak.

## Installation (Resource Pack)

1. Download the repository as ZIP
2. Rename it to `Minecraft Legacy Console Soundpack.zip` (optional)
3. Put the zip in your Minecraft `resourcepacks` folder
4. Load it in-game (it works best as a resource pack if i remember)

## Technical Info

- Originally extracted from Legacy Console Edition
- Ported for modern Minecraft Java Edition
- Folder structure follows standard Minecraft `assets/minecraft/sounds/music/` layout

## Credits & Legal

- All music composed by **C418**
- Original assets belong to Mojang Studios / Microsoft
- This is a **pure preservation / fan project** for nostalgic purposes only.
- For personal use. Do not reupload or monetize.

Made by Freshwaffle
